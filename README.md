...
体积较大（约 3GB+，内含运行时与模型权重），**优先用网盘整包下载**，不必自己编译。

| 渠道 | 说明 |
| --- | --- |
| **[夸克网盘](https://pan.quark.cn/s/1e76f9af231b)** | **推荐**：完整 zip，免合并 |
| [GitHub Releases](https://github.com/DEKVIW/RemovePicWatermark/releases) | 因单文件 2GB 限制，拆成 `.001` / `.002` 分卷，需合并后再解压 |

夸克链接（可复制整段到 App）：

```text
https://pan.quark.cn/s/1e76f9af231b
```

解压后**保留整个文件夹**，运行 `RemovePicWatermark.exe` 或 `start.bat`。
...
- 模型权重默认不在仓库里，请放到 `workspace/models/`（见上文「用到的模型」）。日常使用更建议直接下 [夸克网盘绿色包](https://pan.quark.cn/s/1e76f9af231b)。
